...
export default function HomeScreen() {
  ...
  useFocusEffect(useCallback(() => { loadMatters(); }, []));
  ...
  const onRefresh = async () => { setRefreshing(true); await loadMatters(); setRefreshing(false); };
  ...
  const handleNewMeeting = () => {
    if (matters.length === 0) { setShowNewMatterModal(true); } else { router.push('/record'); }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <MatterCard matter={item} onPress={() => router.push({ pathname: '/matter/[id]', params: { id: item.id } })} />}
        ListEmptyComponent={
      ...
      <View style={styles.footer}><Text style={styles.footerText}>All data stays on your device</Text></View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalButtonCancel} onPress={() => { setShowNewMatterModal(false); setNewMatterName(''); }}>
...
const styles = StyleSheet.create({
  ...
  offlineIndicator: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1a2e1a', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
  dotGreen: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#22c55e', marginRight: 6 },
  offlineText: { color: '#22c55e', fontSize: 12, fontWeight: '500' },
  list: { padding: 20, paddingBottom: 160 },
  ...
  fab: { position: 'absolute', bottom: 100, alignSelf: 'center', backgroundColor: '#3b82f6', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 24, paddingVertical: 14, borderRadius: 30, shadowColor: '#3b82f6', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 },
  ...
  modalTitle: { fontSize: 20, fontWeight: '600', color: '#fff', marginBottom: 8 },
  modalSubtitle: { fontSize: 14, color: '#888', marginBottom: 20 },
  modalInput: { backgroundColor: '#0a0a0a', borderRadius: 12, padding: 16, fontSize: 16, color: '#fff', borderWidth: 1, borderColor: '#333', marginBottom: 20 },
